...
public class BaseAnnotationXmlDeserializer<T> extends AbstractXmlComponent implements XmlDeserializer<T> {
    ...
    public WechatPayBody<T> deserialize(String xml, Class<T> bodyClass) {
        try {
            Document document = DocumentHelper.parseText(xml);
            super.printXML(document);
            Element rootElement = document.getRootElement();
            WechatPayBody wechatPayBody = new WechatPayBody();
            ...
            for (Field field : WechatPayBody.ReturnSuccessResponse.class.getDeclaredFields()) {
                field.setAccessible(true);
                if (field.isAnnotationPresent(XmlElement.class)) {
                    dealXmlElementAnnotation(rootElement, field, returnSuccessResponse);
                }
            }
            T responseXmlBody = makeResponseXmlBody(rootElement, bodyClass);
            returnSuccessResponse.setResponseXmlBody(responseXmlBody);
    ...
    private <E> E makeResponseXmlBody(Element rootElement, Class<E> clazz) throws IllegalAccessException, InstantiationException {
     ...
     */
    @SuppressWarnings("unchecked")
    private void dealXmlElementAnnotation(Element rootElement, Field field, Object entity) {
        ...
        if (Objects.nonNull(element)) {
            Optional<Object> valueOptional = super.elementValue(rootElement, field.getType(), xmlElementAnnotation.name());
     ...
     * @param entity
     */
    private void dealXmlElementArrayAnnotation(Element rootElement, Field field, Object entity) {
        Class<?> clazz = entity.getClass();
        XmlElementArray xmlElementArrayAnnotation = field.getAnnotation(XmlElementArray.class);
        // 1. 获取总数值
        Optional<Object> indexOptional = super.elementValue(rootElement, Integer.class, xmlElementArrayAnnotation.indexElement());
                ...
                for (int i = 0; i < index; i++) {
                    try {
                        ...
                        Arrays.stream(componentType.getDeclaredFields())
                                .filter(componentTypeField -> componentTypeField.isAnnotationPresent(XmlElement.class))
                                .forEach(componentTypeField -> {
                                    XmlElement componentTypeXmlElement = componentTypeField.getAnnotation(XmlElement.class);
                                    // 取得每个数组元素字段的值
                                    Optional<Object> componentTypeFieldValueOptional = super.elementArrayValue(rootElement, componentTypeField.getType(), componentTypeXmlElement, j);
